# Read and write DNS Hostname Info -  DNS-Host-Name-Attributes property set - http://msdn.microsoft.com/en-us/library/ms684331(v=vs.85).aspx
function Set-AdAclDnsInfo {
    <#
        .Synopsis
            The function will delegate the premission for a group to read/write
            DNS Hostname Information Computer objects in an OU
        .DESCRIPTION
            The function will delegate the premission for a group to read/write
            DNS Hostname Information Computer objects in an OU
        .EXAMPLE
    ...
    Process {
        $parameters = @{
            ...
            ObjectType            = $extendedrightsmap['DNS Host Name Attributes']
