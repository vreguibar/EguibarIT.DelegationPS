Function Set-AdAclRenameComputer {
    <#
        .Synopsis
            Set delegation to rename computer
        .DESCRIPTION
            Configures the container (OU) to delegate the permissions to a group so it can rename computer objects.
        .EXAMPLE
            Set-AdAclRenameComputer -Group "SG_SiteAdmins_XXXX" -LDAPPath "OU=Users,OU=XXXX,OU=Sites,DC=EguibarIT,DC=local"
        .EXAMPLE
            Set-AdAclRenameComputer -Group "SG_SiteAdmins_XXXX" -LDAPPath "OU=Users,OU=XXXX,OU=Sites,DC=EguibarIT,DC=local" -RemoveRule
        .PARAMETER Group
    ...
    End {
        ...
        Write-Verbose -Message "Function $($MyInvocation.InvocationName) finished delegating rename computer."
        Write-Verbose -Message ''
        Write-Verbose -Message '--------------------------------------------------------------------------------'
        Write-Verbose -Message ''
    } #end END
