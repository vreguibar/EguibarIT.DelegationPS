...
function Set-AdAclUnlockUser {
    <#
        .Synopsis
            The function will delegate the premission for a group to Unlock a user in an OU
        .DESCRIPTION
            The function will delegate the premission for a group to Unlock a user
        .EXAMPLE
            Set-AdAclUnlockUser -Group "SG_SiteAdmins_XXXX" -LDAPPath "OU=Users,OU=XXXX,OU=Sites,DC=EguibarIT,DC=local"
        .EXAMPLE
            Set-AdAclUnlockUser -Group "SG_SiteAdmins_XXXX" -LDAPPath "OU=Users,OU=XXXX,OU=Sites,DC=EguibarIT,DC=local" -RemoveRule
        .PARAMETER Group
    ...
    Process {
        ...
        If ($PSBoundParameters['RemoveRule']) {
            ...
            if ($PSCmdlet.ShouldProcess($PSBoundParameters['Group'], 'Remove permissions for lockoutTime?')) {
                # Add the parameter to remove the rule
                $Splat.Add('RemoveRule', $true)
            } #end If
        } #end If

        If ($PSCmdlet.ShouldProcess($PSBoundParameters['Group'], 'Delegate the permisssions for lockoutTime?')) {
    ...
    End {
        ...
        Write-Verbose -Message "Function $($MyInvocation.InvocationName) finish unlock user."
        Write-Verbose -Message ''
        Write-Verbose -Message '--------------------------------------------------------------------------------'
        Write-Verbose -Message ''
    } #end END
