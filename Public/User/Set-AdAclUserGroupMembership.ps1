...
function Set-AdAclUserGroupMembership {
    <#
        .Synopsis
            ...
            Group Membership Set of user in an OU
        .DESCRIPTION
            The function will delegate the premission for a group to Modify Group Membership Set of user object
        .EXAMPLE
    ...
    Process {
        ...
        $Splat = @{
            ...
            AccessControlType     = 'Allow'
            ObjectType            = $Variables.ExtendedRightsMap['Group Membership']
        ...
        If ($PSBoundParameters['RemoveRule']) {
            ...
            if ($Force -or $PSCmdlet.ShouldProcess($PSBoundParameters['Group'], 'Remove permissions for Group Membership?')) {
                # Add the parameter to remove the rule
                $Splat.Add('RemoveRule', $true)
            } #end If
        } #end If

        If ($Force -or $PSCmdlet.ShouldProcess($PSBoundParameters['Group'], 'Delegate the permisssions for Group Membership?')) {
    ...
    End {
        ...
        Write-Verbose -Message "Function $($MyInvocation.InvocationName) finished delegation of user group membership."
        Write-Verbose -Message ''
        Write-Verbose -Message '--------------------------------------------------------------------------------'
        Write-Verbose -Message ''
    } #end END
