...
function Set-AdAclUserPublicInfo {
    <#
        .Synopsis
            ...
            Public Information Set of user in an OU
        .DESCRIPTION
            The function will delegate the permission for a group to Modify Public Information Set of user object
        .EXAMPLE
            Set-AdAclUserPublicInfo -Group "SG_SiteAdmins_XXXX" -LDAPPath "OU=Users,OU=XXXX,OU=Sites,DC=EguibarIT,DC=local"
        .EXAMPLE
            Set-AdAclUserPublicInfo -Group "SG_SiteAdmins_XXXX" -LDAPPath "OU=Users,OU=XXXX,OU=Sites,DC=EguibarIT,DC=local" -RemoveRule
        .PARAMETER Group
    ...
    Process {
        ...
        If ($PSBoundParameters['RemoveRule']) {
            ...
            if ($Force -or $PSCmdlet.ShouldProcess($PSBoundParameters['Group'], 'Remove permissions for Public Information?')) {
    ...
    End {
        ...
        $txt = ($Variables.FooterDelegation -f $MyInvocation.InvocationName,
            'delegating of user public info.'
        )
        Write-Verbose -Message $txt
    } #end END
}
