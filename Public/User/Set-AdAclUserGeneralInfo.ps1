...
function Set-AdAclUserGeneralInfo {
    <#
        .Synopsis
            ...
            General Information Set of user in an OU
        .DESCRIPTION
            The function will delegate the premission for a group to Modify General Information Set of user object
        .EXAMPLE
            Set-AdAclUserGeneralInfo -Group "SG_SiteAdmins_XXXX" -LDAPPath "OU=Users,OU=XXXX,OU=Sites,DC=EguibarIT,DC=local"
        .EXAMPLE
            Set-AdAclUserGeneralInfo -Group "SG_SiteAdmins_XXXX" -LDAPPath "OU=Users,OU=XXXX,OU=Sites,DC=EguibarIT,DC=local" -RemoveRule
        .PARAMETER Group
    ...
    Process {
        ...
        $Splat = @{
            ...
            AccessControlType     = 'Allow'
            ObjectType            = $Variables.ExtendedRightsMap['General Information']
        ...
        If ($PSBoundParameters['RemoveRule']) {
            ...
            if ($Force -or $PSCmdlet.ShouldProcess($PSBoundParameters['Group'], 'Remove permissions for General Information?')) {
                # Add the parameter to remove the rule
                $Splat.Add('RemoveRule', $true)
            } #end If
        } #end If

        If ($Force -or $PSCmdlet.ShouldProcess($PSBoundParameters['Group'], 'Delegate the permisssions for General Information?')) {
    ...
    End {
        ...
        Write-Verbose -Message "Function $($MyInvocation.InvocationName) finished delegation of user emageneralil info."
        Write-Verbose -Message ''
        Write-Verbose -Message '--------------------------------------------------------------------------------'
        Write-Verbose -Message ''
    } #end END
