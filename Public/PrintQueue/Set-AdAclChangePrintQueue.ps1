...
function Set-AdAclChangePrintQueue {
    <#
        ...
        .DESCRIPTION
            Configures the container (OU) to delegate the permissions to a group so it can Modify Print Queues objects.
        .EXAMPLE
            Set-AdAclChangePrintQueue -Group "SG_SiteAdmins_XXXX" -LDAPPath "OU=Users,OU=XXXX,OU=Sites,DC=EguibarIT,DC=local"
        .EXAMPLE
            Set-AdAclChangePrintQueue -Group "SG_SiteAdmins_XXXX" -LDAPPath "OU=Users,OU=XXXX,OU=Sites,DC=EguibarIT,DC=local" -RemoveRule
        .PARAMETER Group
    ...
    End {
        ...
        Write-Verbose -Message "Function $($MyInvocation.InvocationName) finished delegating PrintQueue changing."
        Write-Verbose -Message ''
        Write-Verbose -Message '--------------------------------------------------------------------------------'
        Write-Verbose -Message ''
    } #end END
